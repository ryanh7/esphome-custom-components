...
bool parse_xiaomi_value(uint16_t value_type, const uint8_t *data, uint8_t value_length, XiaomiParseResult &result) {
  if ((value_type == 0x100A) && (value_length == 1)) {
    ...
    result.button = encode_uint16(data[0], data[1]);
...
optional<XiaomiParseResult> parse_xiaomi_header(const esp32_ble_tracker::ServiceData &service_data) {
  ...
  result.raw_offset = 5 + (result.has_capability ? 1: 0) + (result.has_mac? 6 : 0);

  const uint16_t device_uuid = encode_uint16(raw[3], raw[2]);

  result.type = XiaomiParseResult::TYPE_REMOTE;
  result.name = "Remote";
...
bool report_xiaomi_results(const optional<XiaomiParseResult> &result, const std::string &address) {
  ...
  if (result->button.has_value()) {
    ESP_LOGD(TAG, "  Button Index: %d", *result->button);
  }

  if (result->action.has_value()) {
    switch (*result->action) {
      case BUTTON_TYPE_CLICK:
        ESP_LOGD(TAG, "  Action: Click");
        break;
      case BUTTON_TYPE_DOUBLE_CLICK:
        ESP_LOGD(TAG, "  Action: Double-click");
        break;
      case BUTTON_TYPE_LONG_PRESS:
        ESP_LOGD(TAG, "  Action: Long press");
        break;
      case BUTTON_TYPE_TRIPLE_CLICK:
        ESP_LOGD(TAG, "  Action: Triple-click");
        break;
      default:
        ESP_LOGD(TAG, "  Action: Unkown(%d)", *result->action);
        break;
    }
  }
